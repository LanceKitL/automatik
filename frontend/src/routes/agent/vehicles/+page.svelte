<script lang="ts">
	...
	import { getAgentVehicles, resolvePhotoUrl } from '$lib/services/api';
	...
	onMount(async () => {
		try {
			const res = await getAgentVehicles();
			vehicles = res.data;
...
<p class="subtitle">Find vehicles for walk-in customers — create inquiries or schedule test drives.</p>
...
{:else}
	<div class="grid">
		{#each filtered as v}
			<button class="card" onclick={() => goto(`/agent/vehicles/${v.vehicle_id}`)}>
				<div class="card-img">
					{#if getPhotoUrl(v, 0)}
...
<style>
	...
	.price-btn:hover { border-color: var(--blue); }
	.price-btn.active { border-color: var(--blue); background: var(--blue-bg); }
	.price-btn-label { font-weight: 600; color: var(--text-dark); }
	.price-btn.active .price-btn-label { color: #1d4ed8; }
	.price-btn-range { color: var(--text-muted); font-size: 12px; }
	.price-btn.active .price-btn-range { color: var(--blue); }
	...
	.modal {
		...
		box-shadow: var(--shadow-lg); z-index: 110; padding: 24px;
		width: 360px; max-width: 90vw;
	}
	.modal h3 { font-size: 18px; font-weight: 700; color: var(--text-dark); margin: 0 0 16px; }
	.modal-fields { display: flex; flex-direction: column; gap: 12px; margin-bottom: 20px; }
	.modal-fields label { display: flex; flex-direction: column; gap: 4px; font-size: 13px; font-weight: 500; color: var(--text-dark); }
	.modal-fields input { padding: 10px 12px; border: 1px solid var(--border); border-radius: var(--radius-md); font-size: 14px; font-family: inherit; outline: none; }
	.modal-fields input:focus { border-color: var(--blue); box-shadow: 0 0 0 2px rgba(59,130,246,0.15); }
	.modal-actions { display: flex; gap: 8px; }
	.modal-actions button { flex: 1; padding: 10px; border: none; border-radius: var(--radius-md); font-size: 14px; font-weight: 600; cursor: pointer; font-family: inherit; transition: background 0.15s; }
	.btn-apply { background: var(--blue); color: var(--text-white); }
	.btn-apply:hover { background: #2563eb; }
	.btn-clear { background: var(--bg-hover); color: var(--text-dark); }
	...
	.pill {
		...
		background: var(--bg-card); font-size: 12px; color: var(--text-dark);
		cursor: pointer; font-family: inherit; transition: all 0.15s;
	}
	.pill:hover { border-color: var(--blue); color: var(--blue); }
	.pill.active { background: var(--blue); color: var(--text-white); border-color: var(--blue); }
	.pill-sep { width: 1px; height: 20px; background: var(--border); margin: 0 4px; }

	.loading, .empty { padding: 48px 0; text-align: center; color: var(--text-muted); font-size: 14px; }

	.grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(260px, 1fr)); gap: 16px; }
	.card {
		background: var(--bg-card); border: 1px solid var(--border); border-radius: var(--radius-md);
		overflow: hidden; cursor: pointer; text-align: left; font-family: inherit;
		font-size: inherit; padding: 0; transition: box-shadow 0.2s;
	}
	.card:hover { box-shadow: 0 4px 14px rgba(0,0,0,0.08); }
	.card-img { height: 180px; background: var(--bg-hover); overflow: hidden; }
	.card-img img { width: 100%; height: 100%; object-fit: cover; }
	.no-photo { height: 180px; display: flex; align-items: center; justify-content: center; color: var(--text-muted); font-size: 13px; background: var(--bg-hover); }
	.card-body { padding: 12px 14px; }
	.card-title { font-size: 15px; font-weight: 600; color: var(--text-dark); margin-bottom: 2px; }
	.card-sub { font-size: 12px; color: var(--text-muted); margin-bottom: 6px; }
	.card-price { font-size: 16px; font-weight: 700; color: var(--blue-dark); margin-bottom: 6px; }
	.card-specs { display: flex; gap: 6px; flex-wrap: wrap; }
	.card-specs span { font-size: 11px; background: var(--bg-hover); padding: 2px 8px; border-radius: var(--radius-sm); color: var(--text-light); text-transform: capitalize; }
</style>
